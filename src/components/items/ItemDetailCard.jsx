import { CalendarDays, Ellipsis, Pencil, Trash2, ChevronDown, RefreshCw } from "lucide-react";
...
function formatItemDate(item) {
  ...
  const timeStr = item.start_time && item.end_time ? `${fmtTime(item.start_time)} – ${fmtTime(item.end_time)}` : null;
...
function ItemDetailCard({
  bucketList, item, canEdit, isOwner, voteScore, userVote, isVoting,
  onBack, onUpvote, onDownvote, onAddToCalendar, onAddDate, onEdit,
  onDelete, onUpdateStatus, onOptionsClick, showBreadcrumb = true,
}) {
  return (
    <article className="item-detail-card">
      <div className="item-detail-hero">
        ...
        <div className="item-detail-topbar">
          ...
          <button type="button" className="item-options-button" onClick={onOptionsClick} aria-label="Item options">
        ...
        <div className="item-detail-votes item-detail-votes-prominent">
          <VoteControls itemTitle={item.title} score={voteScore} activeVote={userVote} isVoting={isVoting} onUpvote={onUpvote} onDownvote={onDownvote} variant="focus" />
        </div>
      </div>
      ...
      <div className="item-meta-row" style={{ alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        ...
        <span className="item-meta-subtle"><RelativeTime timestamp={item.updated_at} /></span>
      ...
      <div className="item-action-row">
        ...
        {canEdit && (
          <>
            ...
            <button type="button" className="item-action-pill item-action-pill-danger" onClick={onDelete}>
              <Trash2 size={13} />
