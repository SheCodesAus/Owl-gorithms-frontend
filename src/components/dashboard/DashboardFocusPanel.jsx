...
import {
  Lock, Globe, CalendarDays, CheckCircle2, Plus, UserPlus,
  ArrowRight, X, Eye, Clock3, Ellipsis, Pencil, Trash2,
  ExternalLink, Copy, Snowflake, Flame,
} from "lucide-react";
...
function getItemScheduleText(item) {
  ...
  let dateText = (startDate && endDate && startDate !== endDate)
    ? `${startDate} → ${endDate}`
    : startDate || endDate || "";
  let timeText = (startTime && endTime && startTime !== endTime)
    ? `${startTime} → ${endTime}`
    : startTime || endTime || "";
...
function DashboardFocusPanel({
  ...
  const isOwner = bucketList.owner?.id && user?.id
    ? Number(bucketList.owner.id) === Number(user.id)
    : false;

  const currentUserMembership = bucketList.memberships?.find(
  ...
  const handleOpenFullPage = () => { navigate(`/bucketlists/${bucketList.id}`); setShowOptionsMenu(false); };
  ...
  return (
    <aside className={shellClass}>
      ...
      >
        {/* ── Sticky header ──────────────────────────────────────────────── */}
        <div className="dashboard-focus-band-inner mx-3 mt-3 shrink-0 flex flex-col gap-4">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
              ...
              <span className="inline-flex items-center gap-1.5 rounded-full bg-black/8 px-3 py-1.5 text-sm text-black/80 backdrop-blur-sm">
                {bucketList.is_public ? <Globe size={15} /> : <Lock size={15} />}
                  ...
                  {isOwner && (
                    <>
                      ...
                      <button type="button" onClick={handleFreeze} className="flex w-full cursor-pointer items-center gap-3 px-4 py-3 text-left text-sm font-medium text-[var(--body-text)] transition hover:bg-black/5">
                        {bucketList.is_frozen ? <Flame size={16} /> : <Snowflake size={16} />}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <Avatar user={item.creator} size="md" className="shrink-0" />
