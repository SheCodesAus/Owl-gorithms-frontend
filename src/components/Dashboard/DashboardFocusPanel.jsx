...
import { Lock, Globe, CalendarDays, CheckCircle2 } from "lucide-react";
    ...
    >
      <AnimatePresence mode="wait">
        ...
        >
          {/* Integrated top band */}
          <motion.div
            className="relative overflow-hidden border-b border-white/12"
          ...
          >
            ...
            <section className="flex-1 pt-5">
              ...
              {recentItems.length ? (
                <div className="space-y-3">
                  {recentItems.map((item, index) => (
                    <motion.div
                      ...
                      initial={{ opacity: 0, y: 8 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.22 + index * 0.08, duration: 0.28 }}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                            ...
                            <p className="shrink-0 text-xs text-black/42 sm:text-sm">
                              <RelativeTime timestamp={item.updated_at} />
                            </p>
                          </div>

                          {item.description ? (
                            <p className="mt-1 line-clamp-2 text-sm text-[var(--muted-text)]">
                              {item.description}
                            </p>
                          ) : null}

                          <p className="mt-2 text-xs font-medium text-[var(--heading-text)] sm:text-sm">
                ...
                >
                  No recent updates yet. Once people start adding or editing items, they’ll show up here.
                </motion.div>
              )}
            </section>
